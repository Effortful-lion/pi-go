...
- 文件命名格式：`YYYY-M-DD.md`（如 `2026-7-28.md`，月不带前导零）
- 文件内容格式：
  ```
  ## YYYY-MM-DD
...
### 12. 设计文档归档
- 每次编码前的设计/计划文档必须写在 `./docs/` 目录下
- 文件命名示例：`./docs/ai模块设计.md`、`./docs/agent运行时设计.md`
- 包括但不限于：架构设计、接口设计、模块职责说明、开发计划
...
### 17. 开发流程
- **每次编码前，先写计划，后归档，最后用户确认后再编写**
- 计划内容包括：要做什么、改动哪些文件、API 设计、预期结果
- 不要未经确认直接开始写代码

### 18. 动作前置检查
- 每次 chat 中执行实质性动作（编码、改文件、生成文档等）前，必须检查两项：
  1. **Git 保存**：当前修改是否已通过 git commit 保存？如果未保存，提醒用户先保存
  2. **文档归档**：本次动作是否需要归档文档（设计文档、变更日志）？如果是，一并完成

### 19. 动作后置约定
- 每次 chat 中执行实质性动作（编码、改文件、生成文档等）前，必须执行两项：
  1. **Git 保存**：当前修改是否已通过 git commit 保存？如果未保存，提醒用户先保存
  2. **文档归档**：本次动作是否需要归档文档（设计文档、变更日志）？如果是，一并完成
